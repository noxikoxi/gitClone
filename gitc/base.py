...
def write_tree():
    ...
    with data.get_index() as index:
        for path, oid in index.items():
            ...
            dirpath = os.path.sep.join(dirpath)
            current = current.setdefault(dirpath, {})
    ...
    def write_tree_recursive(tree_dict):
        ...
        for name, value in tree_dict.items():
            if type(value) is dict:
                if name == "": # Files located next to GIT_DIR
                    for filename, oid in value.items():
                        entries.append((filename, oid, "blob"))
                    continue
                else:
                    type_ = 'tree'
...
def get_tree(oid, base_path=""):
    ...
    for type_, oid, name in _iter_tree_entries(oid):
        ...
        if type_ == "blob":
            ...
            result.update(get_tree(oid, f'{path}{os.path.sep}'))
        else:
...
def _checkout_index(index):
    _empty_current_directory()
    for path, oid in index.items():
        if len(path.split(os.path.sep)) == 1 : # just a file
...
def is_ancestor_of(commit, possible_ancestor):
    return possible_ancestor in iter_commits_and_parents({commit})
...
def get_oid(name):
    ...
    refs_to_try = [
        ...
        f'{os.path.join("refs", "tags", name)}',
        f'{os.path.join("refs", "heads", name)}',
    ]
...
def is_ignored(path):
    return data.GIT_DIR_NAME in os.path.normpath(path).split(os.path.sep)
